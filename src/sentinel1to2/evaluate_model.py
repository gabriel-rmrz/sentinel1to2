  ...
  with torch.no_grad():
    ...
    for inputs, targets, scenes, patch_idx in tqdm(val_loader, "Evaluating batches"):
      ...
      for j in range(min(num_samples, inputs.size(0))):
        ...
        compute_all_metrics(gt_vs_comp_file, scenes[j], ind_from_gt, ind_from_inf, ind_names_from_gt)
        compute_all_metrics(gt_vs_inf_file, scenes[j], target_patch, output_patch, band_names)

        if scene_count < config["evaluation"]["scenes_to_plot"]:

          plot_s2_composites_2d(
              job_plots_dir / 'patches/val/bands/histos2d',
              target_patch,        # (C, H, W)
              band_names,   # e.g. ["b1","blue","green","red","b5",...]
              scenes[j],
          ...
          plot_histo_2d(job_plots_dir / 'patches/val/bands/histos2d', 
              target_patch, 
              band_names, 
              scenes[j], 
              prefix=f"{scene_count}_gt")
          ...
          plot_comparison_rgb_composites_2d(
              job_plots_dir / 'patches/val/bands/histos2d_comparison',
              target_patch,        # (C, H, W)
              output_patch,       # (C, H, W)
              band_names,   # e.g. ["b1","blue","green","red",...,"nir",...]
              scenes[j],
